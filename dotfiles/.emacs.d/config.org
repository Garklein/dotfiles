* todo
- fix piping input when compiling (sometimes, the pipe shouldn't go at the end, like if at the end we rm stuff)
- abbrev-mode
- helm, ivy, ido
- term emulator
...
(use-package evil
  ...
  (defvar default-font-size
    (let* ((xrandr-numbers
	    (mapcar 'string-to-number
		    (split-string
		     (shell-command-to-string "xrandr | grep -w connected | grep -oE '[0-9]+(mm )?x ?[0-9]+' | grep -o '[0-9]*'")  "\n")))
	   (pixelw (nth 0 xrandr-numbers))
	   (mmw (nth 2 xrandr-numbers))
	   (mm/pixel (/ (float pixelw) (float mmw))))
      (round (* 1.8 mm/pixel))))
  (defvar font-size default-font-size)
  (defun set-font-size (new-size)
    (setq font-size new-size)
    (set-frame-font (concat "Agave " (number-to-string font-size)) nil t))

  (advice-add 'text-scale-increase :override
  	      (lambda (inc)
  		(set-font-size (+ font-size inc))))
  (advice-add 'text-scale-decrease :override
  	      (lambda (dec)
  		(set-font-size (- font-size dec))))
  (evil-define-key 'normal 'global (kbd "C-x C-<backspace>")
    (lambda () (interactive) (set-font-size default-font-size)))
  ...
  (evil-define-key 'normal 'global (kbd "C-M-x") 'fzf-directory)
...
(use-package symex
  ...
  :config
  (symex-mode 1)
  (defvar-local want-to-be-in-symex nil
    "Should we be in symex mode")
  (global-set-key (kbd "M-,")
		  (lambda ()
		    (interactive)
		    (if (setf want-to-be-in-symex (not want-to-be-in-symex))
			(symex-mode-interface)
		      (symex-escape-higher))))
  (advice-add 'evil-normal-state :after
	      (lambda (&rest _)
		(when want-to-be-in-symex
		  (symex-mode-interface)))))
...
(defun adjust-frame-transparency (&optional gap-alpha gap-width)
  (modify-all-frames-parameters
   `((alpha-background      . 83)
     (alpha-borders         . ,(or gap-alpha 0))
     (right-divider-width   . ,(or gap-width 20))
     (bottom-divider-width  . ,(or gap-width 20))
     (internal-border-width . 20))))


(defun add-window-margins- ()
  (dolist (f (frame-list))
...
(defun add-window-margins (&rest _)
  (run-with-timer 0.01 nil #'add-window-margins-))
...
(defun set-wallpaper-by-season (&optional season)
  (let ((wallpaper
    ...
    (run (concat "feh --bg-scale ~/dotfiles/dotfiles/wallpapers/" wallpaper))
    (adjust-frame-transparency)))
...
;; https://codeberg.org/emacs-weirdware/exwm-firefox/src/branch/main/lisp/exwm-firefox.el
...
(setq exwm-input-global-keys
  	...
  	([?\s-J] . (lambda () (interactive) (find-file "~/dotfiles/modules/home")))
  	([?\s-K] . (lambda () (interactive) (find-file "~/dotfiles/modules/nixos")))
  	([?\s-L] . (lambda () (interactive) (find-file "~/dotfiles/hosts/home/archlinux")))
  	([?\s-;] . (lambda () (interactive) (async-shell-command "cd ~/dotfiles && sudo nixos-rebuild switch --flake .#bog" "nixos-rebuild switch")))
  	...
  	(,(kbd "<XF86MonBrightnessUp>") . (lambda () (interactive) (run "brightnessctl s +3%") (update-bar)))
  	(,(kbd "<XF86MonBrightnessDown>") . (lambda () (interactive) (run "brightnessctl s 3%-") (update-bar)))

  	([?\s-j] . (lambda () (interactive) (exwm-workspace-switch 0)))
  	([?\s-k] . (lambda () (interactive) (exwm-workspace-switch 1)))
  	([?\s-l] . (lambda () (interactive) (exwm-workspace-switch 2)))))
...
(add-hook 'exwm-mode-hook
	  (lambda () (setq-local evil-normal-state-cursor '(bar . 0))))
#+end_src
* visuals
** theme
#+begin_src elisp
...
(setq-default mode-line-unraised
 	      '((:eval (if buffer-read-only "" "%* "))
	        (:eval (if symex-editing-mode "(%b)" "%b"))
		...
		(:eval
		 (pcase major-mode
		   ('exwm-mode "")
		   ('pdf-view-mode
		    (concat (pdf-view-current-pagelabel) "/" (number-to-string (length (pdf-info-pagelabels)))))
		   (_ "%l-%C")))))

					; get the width with (- (window-total-width) 1)
					; manually insert the right alignment by evalling (it won't propertize the spacer in the middle)
					; use an overline text face
					; add the two spaces on either side
					; might be impossible since we need to get the formatted mode line
(defun bottom-margin (s)
  (if (symbolp s)
      s
    `(:propertize ,s display (raise 1))))
(setq-default mode-line-format
	      (mapcar 'bottom-margin
		      (append '("  ") mode-line-unraised '("  "))))
...
(add-hook 'go-mode-hook
	  (lambda ()
	    (set-command (run-with "go run" "go"))
	    (set-normal-tab 4)
	    (set-pretty '("=" "==" "!=" "*"))))
#+end_src
** uiua
...
(define-generic-mode
  ...
  '("\\.pnlc")
  pnlc-mode-hook)
(add-hook 'pnlc-mode-hook
	    (lambda ()
	      (set-normal-tab 2)
	      (set-command
	       (run-with "~/PNLC/bin/pnlc ~/PNLC/prelude.pnlc" "pnlc"))))
#+end_src
** iogii
#+begin_src elisp
(defvar iogii-mode-hook)
(define-generic-mode
    'iogii-mode
  '("# ")
  nil
  ...
  '("\\.iog$")
  iogii-mode-hook)
(add-hook 'iogii-mode-hook
	    (lambda ()
	      (set-command 
	       (with-file-name "iog" (concat "ruby ~/dormant/iogii/src/main.rb " file-name " -hs > /tmp/a.hs && runhaskell -w /tmp/a.hs")))))
#+end_src
** lean 4
#+begin_src elisp
  (use-package lean4-mode
    ...
    :straight (lean4-mode :type git :host github
                          ...
                          :files ("*.el" "data"))
    :config
    (push '(errors-above . hide) magit-section-initial-visibility-alist)
    (push '(errors-below . hide) magit-section-initial-visibility-alist))
  (advice-add #'lean4-ensure-info-buffer :after
  	    (lambda (&rest _)
	      (with-current-buffer (get-buffer "*Lean Goal*")
		(toggle-truncate-lines 0))))
  (setq lsp-headerline-breadcrumb-enable nil)
(add-hook 'magit-section-set-visibility-hook
	  (lambda (section)
	    (let ((sym (oref section value)))
	      (when (or (eq sym 'errors-below)
			(eq sym 'errors-above))
		'hide))))
#+end_src
** common lisp
...
(add-hook 'scheme-mode-hook
	    (lambda ()
	      (electric-pair-local-mode t)
	      (electric-indent-local-mode 1)
	      (set-command (run-with "racket -f" "scm"))))
#+end_src
** java
#+begin_src elisp
(require 'files)
...
(add-hook 'prolog-mode-hook
	  (lambda ()
	    (set-command (ediprolog-dwim))
	    (evil-local-set-key 'normal (kbd "C-c C-k") (lambda () (interactive) (ediprolog-kill-prolog)))
	    (set-pretty '(":-" "-->"))
	    (set-normal-tab 2)))
(add-to-list 'auto-mode-alist '("\\.pl\\'" . prolog-mode))
(add-to-list 'auto-mode-alist '("\\.scryerrc\\'" . prolog-mode))
(add-to-list 'display-buffer-alist
	     '("\\*ediprolog-consult\\*"
	       (display-buffer-below-selected)))
;; (setq ediprolog-system 'swi)
;; (setq ediprolog-program (string-trim-right (shell-command-to-string "which swipl")))
;; (setq ediprolog-prefix "% ")
(setq ediprolog-system 'scryer)
(setq ediprolog-program (string-trim-right (shell-command-to-string "which scryer-prolog")))
(setq ediprolog-prefix "")
#+end_src
** ocaml
#+begin_src elisp
...
(add-hook 'dired-mode-hook
	    (lambda ()
	      ...
	      (evil-local-set-key 'normal (kbd "C-c C-p") #'dired-preview-global-mode)))
...
;; assume always left to right for performance
(setq-default bidi-display-reordering 'left-to-right
              bidi-paragraph-direction 'left-to-right)
(setq bidi-inhibit-bpa t)

;; skip fontifying during typing
...
(defun clean-up-buffers ()
  ...
  (let ((buffers
    ...
    (dolist (b buffers)
      (unless (or
	       (member (buffer-name b) '("*scratch*" "webcord"))
	       (string-search "Mozilla Firefox" (buffer-name b)))
    	(kill-buffer b)))))
#+end_src
* misc packages
#+begin_src elisp
...
(use-package fzf
  :config
  (advice-add #'fzf :override
	      (lambda (&rest _)
		(fzf-with-command "rg --files" #'fzf--action-find-file (fzf--resolve-directory)))))
#+end_src
* bookmarks

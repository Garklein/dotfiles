...
- make vim marks persist between sessions, make 3j leave a mark
- [i
...
#+begin_src elisp
  ...
  (run "picom")
  (run "feh --bg-center /home/gator/keep/images/wallpapers/fluff-surviv-wallpaper-no-snow-on-bar.png")
  (run "xidlehook --not-when-audio --timer 120 'systemctl suspend' ''")
...
#+begin_src elisp
  (defun screenshot ()
    ...
    (let* ((inhibit-message t)
      ...
      ;; these need to be shell-command because the first must finish before the second
      (shell-command (concat "maim -sq " file))
      ;; this needs to be shell-command since it needs the &> ... because xclip is weird
      (shell-command (concat "xclip -selection clipboard -t image/png " file " &> /dev/null"))))

...
(require 'cuendillar)
;; emacs needs to be a service so that xidlehook can send a message to it
;; to toggle cuendillar and sleep
(server-start)
(defun exwm-sleep ()
  (interactive)
  (lock)
...
(defun set-sleep-minutes (minutes)
  ...
  (run (concat
	  ...
	  " 'cd ~/others/emacs; lib-src/emacsclient --eval \"(exwm-sleep)\"' ''")))
(set-sleep-minutes 2)

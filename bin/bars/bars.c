...
void draw() {
	FORB { xcb_poly_fill_rectangle(c,b->win,b->cleargc,1,(xcb_rectangle_t[]){0,0,b->w,b->h});
		...
		case CLOCK: SH("date +%H:%M:%S","%d:%d:%d",&hh,&mm,&ss); now=hh*hour+mm*60+ss;
			mark(*b,now%halfDay,halfDay,PERP,0); mark(*b,now%hour,hour,PERP+4,0); break;
		case VOL: SH("sndioctl -n output.level output.mute","0.%d\n%d",&percent,&muted);
			if (!muted) mark(*b,1000-percent,1000,PERP,0); break;
		case BATT: SH("apm -bl","%d\n%d",&status,&percent); // status 3 = charging
			mark(*b,percent,100,PERP,(status==3)*4); break;
		case LIGHT: SH("xbacklight -get","%d",&percent); mark(*b,100-percent,100,PERP,0);
		} } xcb_flush(c); }
  ...
  xcb_create_colormap(c,XCB_COLORMAP_ALLOC_NONE,colormap,scr->root,visual);
	...
	FORB {
		...
		xcb_map_window(c, b->win); xcb_configure_window(c,b->win, // make cwm listen
			XCB_CONFIG_WINDOW_X|XCB_CONFIG_WINDOW_Y,(int[]){b->x,b->y});
		xcb_change_property(c,XCB_PROP_MODE_REPLACE,b->win,XCB_ATOM_WM_NAME,XCB_ATOM_STRING,8,4,"bars");
		...
		xcb_create_gc(c,b->gc,b->win,XCB_GC_FOREGROUND|XCB_GC_LINE_WIDTH|XCB_GC_CAP_STYLE,
			(int[]){0xff504c45,5,XCB_CAP_STYLE_ROUND}); }
